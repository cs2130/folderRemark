...
BOOL SQLiteHelper::Open(LPCTSTR lpDbFlie)
{
	if(lpDbFlie == NULL)
	{
		return FALSE;
...
BOOL SQLiteHelper::ExcuteNonQuery(LPCTSTR lpSql)
{
	if(lpSql == NULL)
	{
		return FALSE;
...
BOOL SQLiteHelper::ExcuteNonQuery(LPCTSTR lpSql, int &iRowId)
{
	if(lpSql == NULL)
	{
		return FALSE;
	...
	sqlite3_step(stmt);
	iRowId = sqlite3_last_insert_rowid(m_db);
	return (sqlite3_finalize(stmt) == SQLITE_OK) ? TRUE : FALSE ;
}

BOOL SQLiteHelper::ExcuteNonQuery(SQLiteCommand* pCmd)
{
	if(pCmd == NULL)
	{
		return FALSE;
...
// 查询(回调方式)
BOOL SQLiteHelper::ExcuteQuery(LPCTSTR lpSql,QueryCallback pCallBack)
{
...
// 查询
SQLiteDataReader SQLiteHelper::ExcuteQuery(LPCTSTR lpSql)
{
	if(lpSql == NULL)
	{
		return FALSE;
...
// 获取上一条错误信息
LPCTSTR SQLiteHelper::GetLastErrorMsg()
{
#ifdef UNICODE 
...
SQLiteCommand::SQLiteCommand(SQLiteHelper* pSqlite):
m_pSqlite(pSqlite),
m_pStmt(NULL)
{
}

SQLiteCommand::SQLiteCommand(SQLiteHelper* pSqlite,LPCTSTR lpSql):
m_pSqlite(pSqlite),
m_pStmt(NULL)
{
