...
public:
    CShadowWnd() {
		m_di.sDrawString = _T("file='Menu\\menu_bk.png' corner='40,8,8,8'");
	};
...
public:
    ...
    void Init(CControlUI* pOwner, POINT pt, CPaintManagerUI* pMainPaintManager) {
        //if( pOwner == NULL ) return;
        m_pOwner = pOwner;
        m_ptPos = pt;
        Create((m_pOwner == NULL) ? pMainPaintManager->GetPaintWindow() : m_pOwner->GetManager()->GetPaintWindow(), NULL, WS_VISIBLE | WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, WS_EX_TOOLWINDOW | WS_EX_TOPMOST);
    ...
    void Notify(TNotifyUI& msg)
    {
        if( msg.sType == _T("itemselect") ) {
			...
			//::MessageBox(NULL, "killfocus", _T("ÌבÊ¾(by tojen)"), MB_OK);
			//Close();
    ...
    HWND Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, int x = CW_USEDEFAULT, int y = CW_USEDEFAULT, int cx = CW_USEDEFAULT, int cy = CW_USEDEFAULT, HMENU hMenu = NULL)
    {
        ...
        m_pShadowWnd->Create(hwndParent, _T(""), WS_VISIBLE | WS_POPUP | WS_CLIPSIBLINGS, WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TOPMOST, x, y, cx, cy, NULL);
